...
import { Palette, Ruler, Layers, Move, ArrowRight, ChevronLeft, Plus, Minus, Lock, X, Sparkles } from 'lucide-react';
...
const PRESET_COLORS = [
  '#E0E0E0', '#1A1A1A', '#FFFFFF', '#EFBB76', '#8B4513', 
    ...
    >
      <header className="mb-12 text-center relative">
        ...
        <p className="text-black/40 text-lg max-w-xl mx-auto">
          {isRegenerateMode 
            ...
            : 'Customize your rug by selecting colors, materials, size, and construction — built for real production through RugVision™.'}
        </p>
      </header>
      ...
      <div className="space-y-12">
        ...
        <section className={`bg-black/5 p-8 rounded-3xl border border-black/10 shadow-sm ${isRegenerateMode ? 'opacity-50 pointer-events-none' : ''}`}>
          <label className="flex items-center gap-2 text-xs font-bold tracking-widest uppercase text-black mb-4">
            <Sparkles className="w-4 h-4" /> Design Concept
          </label>
          <textarea
            value={config.prompt}
            onChange={(e) => setConfig({ ...config, prompt: e.target.value })}
            placeholder="Describe your rug design..."
            className="w-full bg-white border border-black/10 rounded-2xl p-6 text-sm font-medium focus:outline-none focus:ring-4 focus:ring-black/20 transition-all min-h-[120px] resize-none"
          />
        </section>
        ...
        <section className="bg-black/5 p-8 rounded-3xl border border-black/10 shadow-sm flex flex-col">
          <div className="flex items-center justify-between mb-8">
            <label className="flex items-center gap-2 text-xs font-bold tracking-widest uppercase text-black">
            ...
            <div className="flex items-center gap-4">
              <button 
                ...
                className="px-4 py-2 bg-white border border-black/10 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:border-blue-600 hover:text-blue-600 disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-sm flex items-center gap-2"
              >
