    ...
    >
                  ...
                  imageUrls.map((url, i) => (
                    <div key={`rug-container-${i}`} className="flex flex-col gap-4">
                      <motion.div
                        ...
                        className={`relative group rounded-3xl overflow-hidden border transition-all duration-500 ${aspectClass} bg-white ${selectedVariation === i ? 'ring-4 ring-black border-transparent scale-[1.02] z-10 shadow-2xl' : 'border-black/10 hover:border-black/30'}`}
                      >
                        ...
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-3 p-6">
                          <button 
                            ...
                            className={`w-full py-3 px-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${selectedVariation === i ? 'bg-black text-white' : 'btn-primary'}`}
                          >
                            {selectedVariation === i ? (
                              <>
          ...
          <div className="max-w-6xl mx-auto mt-16 mb-24 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 border-t border-black/5 pt-12">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[10px] font-bold text-black uppercase tracking-widest">${rate.toFixed(2)} / SQFT</p>
              </div>
            </div>
          ...
          >
            <div className="absolute top-8 right-8 flex items-center gap-4 bg-white/10 backdrop-blur-xl px-6 py-3 rounded-full border border-white/10 z-[210] shadow-2xl text-white">
              <div className="flex items-center gap-3 pr-4 border-r border-white/10">
                <button onClick={(e) => { e.stopPropagation(); setModalZoom(z => Math.max(50, z - 25)); }} className="text-white/60 hover:text-white transition-colors"><Minus className="w-5 h-5" /></button>
                <input type="range" min="50" max="300" value={modalZoom} onClick={e => e.stopPropagation()} onChange={(e) => setModalZoom(parseInt(e.target.value))} className="w-24 accent-white cursor-pointer" />
                <button onClick={(e) => { e.stopPropagation(); setModalZoom(z => Math.min(300, z + 25)); }} className="text-white/60 hover:text-white transition-colors"><Plus className="w-5 h-5" /></button>
              </div>
              <span className="text-xs font-mono font-bold text-white w-12 text-center">{modalZoom}%</span>
            </div>
      ...
      {selectedVariation !== null && (
        <motion.div 
          initial={{ y: 100 }}
          animate={{ y: 0 }}
          className="bg-white/80 backdrop-blur-xl border-t border-black/5 p-6 flex items-center justify-between shadow-2xl z-50"
        >
